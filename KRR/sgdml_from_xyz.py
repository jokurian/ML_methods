...
# Argument parser to accept units as arguments
parser = argparse.ArgumentParser(description='Creates a dataset from extended XYZ format.')
parser.add_argument('dataset', metavar='<dataset>', type=argparse.FileType('r'), help='path to extended xyz dataset file')
parser.add_argument('-o', '--overwrite', dest='overwrite', action='store_true', help='overwrite existing dataset file')
parser.add_argument('--r_unit', type=str, help="Description of the length unit, e.g. 'Ang' or 'au'", required=True)
parser.add_argument('--e_unit', type=str, help="Description of the energy unit, e.g. 'kcal/mol' or 'eV'", required=True)
...
if is_extxyz:
    ...
    if not all_z_the_same:
        sys.exit(
            ui.color_str('[FAIL]', fore_color=ui.RED, bold=True)
            + ' Order of atoms changes across dataset.'
        )
...
# Use the units from the arguments instead of interactive inputs
base_vars['r_unit'] = args.r_unit
base_vars['e_unit'] = args.e_unit
